...
<div class="form-group {{ $errors->has('price') ? 'has-error' : '' }}">
    ...
    <input type="number" name="quantity" value="{{$product->quantity}}" min="1" value="" placeholder="1" name="quantity" class="form-control border-input">    
    <span class="text-danger">{{ $errors->has('quantity') ? $errors->first('quantity') : '' }}</span>
</div>


<div class="form-group {{ $errors->has('price') ? 'has-error' : '' }}">
    {{ Form::label('available', 'Available') }}<br>
    Yes <input type="radio" {{$product->available==1?'checked':''}} checked name="available" value="1">    
    No <input type="radio" {{$product->available==0?'checked':''}} name="available" value="0">        
    <span class="text-danger">{{ $errors->has('available') ? $errors->first('available') : '' }}</span>
</div>
